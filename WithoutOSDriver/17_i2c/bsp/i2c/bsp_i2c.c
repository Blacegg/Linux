...
{
    if ((((base->I2CR) & (1 << 5)) == 0) && ((base->I2SR) & (1 << 5)))
        return 1;
    base->I2CR |= ((1 << 4) | (1 << 2));
...
unsigned char i2c_mater_stop(I2C_Type *base)
{
    ...
    while ((base->I2SR) & (1 << 5))
    {
        ...
        if (timeout == 0)
        {
            return I2C_STATUS_TIMEROUT;
        }
    }
    return I2C_STATUS_OK;
}

void i2c_master_writer(I2C_Type *base,
...
unsigned char i2c_master_transfer(I2C_Type *base, struct i2c_transfer *xfer)
{
    ...
    if ((xfer->direction == kI2C_Write) && (xfer->datasize > 0))
        i2c_master_writer(base, xfer->data, xfer->datasize);
